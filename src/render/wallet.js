const { colors, formatUsd, pad, shortAddress, truncate } = require("../utils/format");

function renderWalletDashboard(result, config) {
  console.clear();
  console.log(colors.cyan("GMGN Wallet Tracker"));
  console.log(colors.dim(`updated=${new Date(result.updatedAt).toLocaleTimeString()} refresh=${config.walletRefreshSeconds}s`));
  console.log("");

  if (!result.configured) {
    console.log(colors.yellow("No wallets configured. Set WALLET_ADDRESSES in .env."));
    return;
  }
...
function renderWalletSection(wallet) {
  console.log(colors.bold(`WALLET ${shortAddress(wallet.address)}  ${wallet.address}`));
  console.log(`SOL balance: ${formatSolBalance(wallet.solBalance)} | Token value: ${formatUsd(wallet.totalTokenValueUsd)} | Total value: ${formatUsd(wallet.totalValueUsd)}`);

  if (wallet.error) {
    console.log(colors.yellow("Syncing with GMGN networks... checking wallet status..."));
  ...
  wallet.holdings.forEach((holding, index) => {
    console.log(
      [
        ...
        pad(truncate(String(holding.symbol || "UNKNOWN").toUpperCase(), 14), 14, "left"),
...
function formatSolBalance(solBalance) {
  ...
  const value = Number.isFinite(solBalance.valueUsd) ? ` / ${formatUsd(solBalance.valueUsd)}` : "";
...
function formatUnrealized(holding) {
  ...
  const pct = Number.isFinite(holding.unrealizedPnlPct) ? `${signed(holding.unrealizedPnlPct)}%` : "-";
  const text = `${usd} / ${pct}`;
  if (!Number.isFinite(holding.unrealizedPnlUsd) && !Number.isFinite(holding.unrealizedPnlPct)) return text;
  const marker = Number.isFinite(holding.unrealizedPnlUsd) ? holding.unrealizedPnlUsd : holding.unrealizedPnlPct;
